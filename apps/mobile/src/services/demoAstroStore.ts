...
import { computeNatalChartLocal, todaysMoon, quickSunSignLocal } from "../lib/natalChart";
import { computeTransitsLocal } from "../lib/transits";

const BIRTH_KEY = "@lumina/demo-birth";
...
let birthCache: DemoBirth | null = null;

export const demoAstroStore = {
  async getBirth(): Promise<DemoBirth | null> {
    if (birthCache) return birthCache;
    try {
      const raw = await AsyncStorage.getItem(BIRTH_KEY);
      birthCache = raw ? (JSON.parse(raw) as DemoBirth) : null;
    } catch {
      birthCache = null;
    }
    return birthCache;
  ...
  async saveBirth(b: DemoBirth): Promise<void> {
    birthCache = b;
    try {
      await AsyncStorage.setItem(BIRTH_KEY, JSON.stringify(b));
    } catch {
      // best-effort
    }
  },

  async clearBirth(): Promise<void> {
    birthCache = null;
    try {
      await AsyncStorage.removeItem(BIRTH_KEY);
    } catch {
      // ignore
    }
  },
  ...
  async getNatalChart(): Promise<NatalChart | null> {
    const b = await demoAstroStore.getBirth();
    if (!b) return null;
    return computeNatalChartLocal(b);
  },

  async getDailyHoroscope(): Promise<DailyHoroscope> {
    const b = await demoAstroStore.getBirth();
    const natal = b ? computeNatalChartLocal(b) : null;
    const sunSign: ZodiacSign | null = b ? quickSunSignLocal(b.birthDate) : null;
    const today = new Date();
    const { sign: moonSign, phase: moonPhase } = todaysMoon(today);
    const dateStr = today.toISOString().slice(0, 10);

    const aspects = natal
      ? findAspects(natal.placements, computeTransitsLocal(today)).slice(0, 8)
      : [];
    ...
    const seed = seedFromDate(dateStr) + (sunSign ? signIdx(sunSign) * 7 : 0) + signIdx(moonSign);
    const slices = HOROSCOPE_DOMAINS.map((d, i) =>
      buildSlice(d, sunSign ?? moonSign, moonSign, moonPhase, seed + i * 13, aspectByDomain[d]),
    );

    const headline = buildHeadline(sunSign, moonSign, moonPhase, seed, aspects[0] ?? null);
...
function signIdx(s: ZodiacSign): number {
  return ZODIAC.indexOf(s);
}
...
function buildSlice(
  ...
  const filtered = aspect ? pool.filter((p) => matchesQuality(p.vibe, aspect.quality)) : pool;
